...
export class MentionEditing extends Plugin {
  ...
  init() {
    ...
    doc.registerPostFixer((writer) =>
      removePartialMentionPostFixer(writer, doc, model.schema)
    );
    doc.registerPostFixer((writer) =>
      extendAttributeOnMentionPostFixer(writer, doc)
    );
    doc.registerPostFixer((writer) =>
      selectionMentionAttributePostFixer(writer, doc)
    );
...
function shouldNotTypeWithMentionAt(position) {
  ...
  const isAfterAMention =
    position.nodeBefore && position.nodeBefore.is('$text');
...
function removePartialMentionPostFixer(writer, doc, schema) {
  ...
  for (const change of changes) {
    ...
    if (change.name == '$text') {
      const nodeAfterInsertedTextNode =
        position.textNode && position.textNode.nextSibling;
    ...
    if (change.type == 'insert' && schema.isInline(change.name)) {
      const nodeAfterInserted =
        position.nodeAfter && position.nodeAfter.nextSibling;
