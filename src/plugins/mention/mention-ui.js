...
import { MentionsView } from './ui/mentions-view';
import { DomWrapperView } from './ui/dom-wrapper-view';
import { MentionListItemView } from './ui/mention-list-item-view';
...
export class MentionUI extends Plugin {
  ...
  init() {
    ...
    const commitKeys =
      editor.config.get('mention.commitKeys') || defaultCommitKeyCodes;
    ...
    for (const mentionDescription of feeds) {
      ...
      const feedCallback =
        typeof feed == 'function'
          ? feed.bind(this.editor)
          : createFeedCallback(feed);
    ...
    this.on('requestFeed:response', (evt, data) =>
  ...
  _createMentionView() {
    ...
    mentionsView.items.bindTo(this._items).using((data) => {
      ...
      const dropdownLimit =
        this.editor.config.get('mention.dropdownLimit') || 10;
  ...
  _setupTextWatcher(feeds) {
    ...
    const watcher = new TextWatcher(
      editor.model,
      createTestCallback(feedsWithPattern)
    );

    watcher.on('matched', (evt, data) => {
      const markerDefinition = getLastValidMarkerInText(
        feedsWithPattern,
        data.text
      );
      ...
      const markerPosition = editor.model.createPositionAt(
        focus.parent,
        markerDefinition.position
      );

      if (
        isPositionInExistingMention(focus) ||
        isMarkerInExistingMention(markerPosition)
      ) {
        this._hideUIAndRemoveMarker();
      ...
      const matchedTextLength =
        markerDefinition.marker.length + feedText.length;
...
function getLastValidMarkerInText(feedsWithPattern, text) {
  ...
  for (const feed of feedsWithPattern) {
    ...
    if (
      currentMarkerLastIndex > 0 &&
      !text.substring(currentMarkerLastIndex - 1).match(feed.pattern)
    ) {
      continue;
    }
    ...
    if (
      !lastValidMarker ||
      currentMarkerLastIndex >= lastValidMarker.position
    ) {
      lastValidMarker = {
        marker: feed.marker,
...
function isPositionInExistingMention(position) {
  ...
  const hasMention =
    position.textNode && position.textNode.hasAttribute('mention');

  const nodeBefore = position.nodeBefore;

  return (
    hasMention ||
    (nodeBefore && nodeBefore.is('$text') && nodeBefore.hasAttribute('mention'))
  );
}
...
function isMarkerInExistingMention(markerPosition) {
  ...
  return (
    nodeAfter && nodeAfter.is('$text') && nodeAfter.hasAttribute('mention')
  );
}
