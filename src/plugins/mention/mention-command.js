...
import { _addMentionAttributes } from './mention-editing';
import { generateMention } from './mention-utils';
...
export class MentionCommand extends Command {
  ...
  execute(options) {
    ...
    const mention = _addMentionAttributes(
      { _text: mentionText, id: mentionID },
      mentionData
    );
    ...
    model.change((writer) => {
      generateMention(mention?.type ?? 'text', {
        selection,
        model,
        writer,
        range,
